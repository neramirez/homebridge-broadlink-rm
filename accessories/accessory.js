...
class BroadlinkRMAccessory extends HomebridgeAccessory {
    ...
    constructor(log, config = {}, serviceManagerType) {
        ...
        log(`\x1b[33m[DEBUG]\x1b[0m ${config.name} CONFIG (${JSON.stringify(config)})`);
        ...
        switch (this.config.logLevel) {
            ...
            default:
                ...
                if (this.config.logLevel !== undefined) {
                    log(`\x1b[31m[CONFIG ERROR] \x1b[33mlogLevel\x1b[0m should be one of: trace, debug, info, warning, error, critical, or none.`);
                }
                this.logLevel = 2;
                break;
        }
    ...
    async performSend(data, actionCallback) {
        ...
        log(`\x1b[33m[DEBUG]\x1b[0m ${name} config (${JSON.stringify(config)})`);

        const {withDelay} = config;
        //Error catch
        if (data === undefined) {
            return
        }
        ...
        if (typeof data === 'string') {
            sendData({host, hexData: data, log, name, logLevel, withDelay});
